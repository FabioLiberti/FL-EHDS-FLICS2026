#!/usr/bin/env python3
"""
FL-EHDS Imaging — Article 71 Opt-Out (GDPR data erasure impact).

Simulates EHDS citizen opt-out on imaging datasets by removing a fraction
of training samples per client and measuring accuracy/fairness degradation.
Mirrors tabular opt-out experiments (run_tabular_optout.py) for cross-modality comparison.

Experiments: 3 algos x 3 datasets x 5 rates x 3 seeds = 135 experiments
Estimated runtime: ~2-3 hours on A100, ~6-8 hours on T4

Usage:
    cd fl-ehds-framework
    python -m benchmarks.run_imaging_optout [--quick] [--fresh] [--dataset chest_xray]

Output: benchmarks/checkpoint_imaging_optout.json
...
CHECKPOINT_FILE = "checkpoint_imaging_optout.json"
LOG_FILE = "experiment_imaging_optout.log"
TRAINER_STATE_FILE = ".trainer_state_optout.pt"
...
def build_experiments(datasets=None, rates=None):
    experiments = []
    ds_list = datasets if datasets else list(IMAGING_DATASETS.keys())
    rate_list = rates if rates else OPTOUT_RATES
...
def experiment_key(ds_name, algo, rate, seed):
    return f"{ds_name}_{algo}_opt{int(rate*100)}_s{seed}"
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=str(OUTPUT_DIR), prefix=".ckpt_optout_", suffix=".tmp"
    )
    try:
...
def apply_optout_imaging(trainer, optout_rate, seed):
    """
    Simulate Article 71 opt-out by removing a fraction of training
    images from each client. Test data is NOT modified.
    Uses deterministic RNG per client: seed * 1000 + cid.
    """
    if optout_rate <= 0:
...
def run_single_imaging(
    ...
    log(f"  Opt-out {optout_rate:.0%}: removed {optout_info['removed']}/{optout_info['total']} "
    ...
    opt_label = f"opt={int(optout_rate*100)}%"
    ...
    for r in range(start_round, num_rounds):
        ...
        # Save after EVERY round: trainer state + JSON checkpoint
...
def download_datasets():
    ...
    # Clean macOS ._* files from all data dirs
    removed = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-EHDS Imaging — Article 71 Opt-Out Evaluation"
    )
    ...
    if checkpoint_data is None:
        checkpoint_data = {
            ...
            "metadata": {
                ...
                "purpose": "Imaging Article 71 opt-out: GDPR data erasure impact on ResNet18",
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Per riprendere: python -m benchmarks.run_imaging_optout{ds_arg}")
        sys.exit(0)
    ...
    log(f"\n{'='*70}")
    log(f"  FL-EHDS Imaging — Article 71 Opt-Out ({mode})")
    ...
    log(f"\n  Opt-Out Impact Summary (Best Accuracy, mean over seeds):")
