#!/usr/bin/env python3
"""
FL-EHDS Imaging — Byzantine Robustness Evaluation.
...
model accuracy and whether defense mechanisms (Krum) protect training.

Experiments: 3 algos x 3 datasets x 4 scenarios x 3 seeds = 108 experiments
Estimated runtime: ~2.5-3 hours on RTX 4090, ~8-10 hours on T4

Usage:
    cd fl-ehds-framework
    python -m benchmarks.run_imaging_byzantine [--download] [--fresh] [--dataset chest_xray]

Output: benchmarks/checkpoint_imaging_byzantine.json
...
def experiment_key(ds_name, algo, attack_label, seed):
    return f"{ds_name}_{algo}_{attack_label}_s{seed}"
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=str(OUTPUT_DIR), prefix=".ckpt_byz_", suffix=".tmp"
    )
    try:
...
def run_single_byzantine(
    ...
    attack_label: str, attack_type: Optional[str], seed: int,
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-EHDS Imaging — Byzantine Robustness Evaluation"
    )
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Per riprendere: python -m benchmarks.run_imaging_byzantine{ds_arg}")
        sys.exit(0)
    ...
    log(f"\n{'='*70}")
    log(f"  FL-EHDS Imaging — Byzantine Robustness ({mode})")
    ...
    for exp_idx, (attack_label, attack_type, ds_name, algo, seed) in enumerate(experiments, 1):
        key = experiment_key(ds_name, algo, attack_label, seed)
        ...
        log(f"\n--- [{exp_idx}/{total_exps}] {ds_name} | {algo} | {attack_label} | seed={seed} ---")
        ...
        try:
            ...
            log(f"--- Done: {ds_name} | {algo} | {attack_label} | s{seed} | "
    ...
    log(f"\n  Byzantine Robustness Summary (Best Accuracy, mean over seeds):")
    attacks = [a[0] for a in ATTACK_SCENARIOS]
    header = f"  {'Dataset':<14} {'Algorithm':<10}" + "".join(f" {a:>12}" for a in attacks)
    log(header)
    log(f"  {'-'*len(header)}")
    ...
    for ds_name in ds_list:
        for algo in ALGORITHMS:
            ...
            for attack in attacks:
                accs = []
                for seed in SEEDS:
                    k = experiment_key(ds_name, algo, attack, seed)
                    r = completed.get(k, {})
