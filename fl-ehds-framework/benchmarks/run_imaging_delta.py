#!/usr/bin/env python3
"""
FL-EHDS Imaging Delta Runner — FedLESAM + HPFL only.

Runs ONLY the 2 new algorithms (not present in V1) on 3 imaging datasets,
with speed optimizations to complete in ~6-8h instead of days.

V1 checkpoint already has: FedAvg, FedProx, Ditto, FedLC, FedExP + others
This delta adds: FedLESAM (2024), HPFL (2025)

Speed optimizations vs V2 full run:
  - freeze_level=2 (freeze conv1+bn1+layer1) -> fewer trainable params
  - local_epochs=2 (vs 3) -> 33% less per-round training
  - patience=4 (vs 6) -> earlier stopping
  - num_rounds=20 (vs 25) -> cap on max rounds

Total: 2 algos x 3 datasets x 3 seeds = 18 experiments (~20-30 min each)

Usage:
    cd fl-ehds-framework
    python -m benchmarks.run_imaging_delta [--resume] [--quick]

Output: benchmarks/paper_results_delta/checkpoint_delta.json
...
# Only the 2 NEW algorithms not in V1
ALGORITHMS = ["FedLESAM", "HPFL"]
...
CHECKPOINT_FILE = "checkpoint_delta.json"
LOG_FILE = "experiment_delta.log"
...
# Speed-optimized imaging config
IMAGING_CONFIG = dict(
    num_clients=5,
    num_rounds=20,          # Reduced from 25: ES kicks in at 10-21 anyway
    local_epochs=2,         # Reduced from 3: 33% faster per round
    batch_size=32,
    ...
    freeze_level=2,         # Freeze conv1+bn1+layer1: ~1.5x faster, better transfer
    use_fedbn=True,
...
EARLY_STOPPING = dict(
    enabled=True,
    patience=4,             # Reduced from 6: faster termination
    min_delta=0.003,
    min_rounds=8,           # Reduced from 10
    metric="accuracy",
)
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(dir=str(OUTPUT_DIR), prefix=".ckpt_delta_", suffix=".tmp")
    try:
...
def main():
    parser = argparse.ArgumentParser(description="FL-EHDS Imaging Delta (FedLESAM + HPFL)")
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Per riprendere: python -m benchmarks.run_imaging_delta --resume")
        sys.exit(0)
    ...
    log(f"\n{'='*66}")
    log(f"  FL-EHDS Imaging Delta — FedLESAM + HPFL")
    log(f"  {total_exps} experiments = {len(IMAGING_DATASETS)} datasets x "
    ...
    if args.quick:
        log(f"  Mode:         QUICK (1 seed, 3 rounds)")
    log(f"{'='*66}\n")
