#!/usr/bin/env python3
"""
FL-EHDS Imaging — 10-Seed Statistical Validation.

Runs FedAvg/Ditto/HPFL × 3 datasets × 10 seeds for Wilcoxon signed-rank test
on imaging (same approach as tabular Phase 4).

Experiments: 3 algos x 3 datasets x 10 seeds = 90 experiments
Auto-resumes from checkpoint (e.g. 40/90 from Colab).

Estimated runtime: ~2.5 hours on RTX 4090, ~5 hours on T4

Usage:
    cd fl-ehds-framework
    python -m benchmarks.run_imaging_seeds10 [--download] [--fresh] [--dataset chest_xray]

Output: benchmarks/checkpoint_imaging_seeds10.json
...
CHECKPOINT_FILE = "checkpoint_imaging_seeds10.json"
LOG_FILE = "experiment_imaging_seeds10.log"
TRAINER_STATE_FILE = ".trainer_state_seeds10.pt"
...
def experiment_key(ds_name, algo, seed):
    return f"{ds_name}_{algo}_s{seed}"
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=str(OUTPUT_DIR), prefix=".ckpt_s10_", suffix=".tmp"
    )
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-EHDS Imaging — 10-Seed Statistical Validation"
    )
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Per riprendere: python -m benchmarks.run_imaging_seeds10{ds_arg}")
        sys.exit(0)
    ...
    log(f"\n{'='*70}")
    log(f"  FL-EHDS Imaging — 10-Seed Statistical Validation ({mode})")
    log(f"  {total_exps} experiments = {len(ALGORITHMS)} algos x {len(IMAGING_DATASETS) if not ds_filter else len(ds_filter)} DS x {len(SEEDS)} seeds")
    ...
    for exp_idx, (ds_name, algo, seed) in enumerate(experiments, 1):
        key = experiment_key(ds_name, algo, seed)
    ...
    log(f"\n  10-Seed Summary (Best Accuracy, mean ± std):")
    header = f"  {'Dataset':<14} {'Algorithm':<10} {'Mean':>8} {'Std':>8} {'Min':>8} {'Max':>8} {'ES':>4}"
    log(header)
    log(f"  {'-'*len(header)}")
    ...
    for ds_name in ds_list:
        for algo in ALGORITHMS:
            ...
            es_count = 0
            for seed in SEEDS:
                k = experiment_key(ds_name, algo, seed)
                r = completed.get(k, {})
                if "error" not in r and r:
                    ...
                    if r.get("stopped_early"):
                        es_count += 1
            if accs:
                log(f"  {ds_name:<14} {algo:<10} {100*np.mean(accs):>7.1f}% {100*np.std(accs):>7.1f}% "
                    f"{100*min(accs):>7.1f}% {100*max(accs):>7.1f}% {es_count:>3}x")
            else:
                log(f"  {ds_name:<14} {algo:<10} {'--':>8} {'--':>8} {'--':>8} {'--':>8} {'--':>4}")
    ...
    try:
        ...
        for ds_name in ds_list:
            ...
            fedavg_accs = []
            for seed in SEEDS:
                h = completed.get(f"{ds_name}_HPFL_s{seed}", {})
                f = completed.get(f"{ds_name}_FedAvg_s{seed}", {})
            ...
            if len(hpfl_accs) >= 5:
                ...
                log(f"  {ds_name:<14} HPFL-FedAvg = {diff:+.1f}pp  p={p:.4f}  {sig}")
            else:
                log(f"  {ds_name:<14} insufficient data ({len(hpfl_accs)} pairs)")
    except ImportError:
