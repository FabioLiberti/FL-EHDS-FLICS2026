#!/usr/bin/env python3
"""
FL-EHDS Imaging — Extend to 5 Seeds (add seed 789, 999).

Adds 2 extra seeds to ALL imaging algo x dataset combinations for
statistical consistency with tabular experiments (5 seeds each).

Experiments: 4 algos x 3 datasets x 2 new seeds = 24 experiments
Estimated runtime: ~12-18 hours

Usage:
    cd fl-ehds-framework
    python -m benchmarks.run_imaging_seeds5 [--quick] [--fresh]

Output: benchmarks/paper_results_delta/checkpoint_imaging_seeds5.json
...
# Only the 2 NEW seeds (existing 42, 123, 456 already done)
NEW_SEEDS = [789, 999]

# All algo x dataset combinations tested in previous experiments
EXPERIMENTS_TODO = []
for ds_name, data_subdir, num_classes, short in [
    ("chest_xray", "chest_xray", 2, "CX"),
    ("Brain_Tumor", "Brain_Tumor", 4, "BT"),
    ("Skin_Cancer", "Skin Cancer", 2, "SC"),
]:
    for algo in ["FedAvg", "Ditto", "HPFL", "FedLESAM"]:
        for seed in NEW_SEEDS:
            EXPERIMENTS_TODO.append((ds_name, data_subdir, algo, seed, num_classes, short))

OUTPUT_DIR = FRAMEWORK_DIR / "benchmarks" / "paper_results_delta"
CHECKPOINT_FILE = "checkpoint_imaging_seeds5.json"
LOG_FILE = "experiment_imaging_seeds5.log"
TRAINER_STATE_FILE = ".trainer_state_seeds5.pt"

# Same config as all other imaging experiments
IMAGING_CONFIG = dict(
    num_clients=5,
    num_rounds=20,
    local_epochs=2,
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=str(OUTPUT_DIR), prefix=".ckpt_s5_", suffix=".tmp"
    )
    try:
...
def run_single_imaging(
    ds_name: str, data_dir: str, algorithm: str, seed: int,
    ...
    if (in_prog and in_prog.get("key") == exp_key
        ...
        try:
            ...
            # Restore early stopping state from saved history
            if es and history:
                for h in history:
    ...
    for r in range(start_round, num_rounds):
        ...
        # Per-client training metrics from this round
        client_metrics = [
            {
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-EHDS Imaging — Extend to 5 Seeds (add 789, 999)"
    )
    parser.add_argument("--fresh", action="store_true", help="Discard checkpoint, start fresh")
    parser.add_argument("--quick", action="store_true", help="Quick validation (3 rounds)")
    ...
    if checkpoint_data is None:
        checkpoint_data = {
            ...
            "metadata": {
                ...
                "purpose": "Extend imaging to 5 seeds (add 789, 999) for statistical consistency with tabular",
                "new_seeds": NEW_SEEDS,
                "algorithms": ["FedAvg", "Ditto", "HPFL", "FedLESAM"],
                "datasets": ["chest_xray", "Brain_Tumor", "Skin_Cancer"],
                "start_time": datetime.now().isoformat(),
                "last_save": None,
                "version": "imaging_seeds5_v1",
            }
        }

    # Graceful shutdown — save checkpoint before exit
    _interrupted = [False]
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Per riprendere: python -m benchmarks.run_imaging_seeds5")
        sys.exit(0)
    ...
    log(f"\n{'='*66}")
    log(f"  FL-EHDS Imaging — 5-Seed Extension ({mode})")
    log(f"  {total_exps} experiments = 4 algos x 3 datasets x 2 new seeds")
    log(f"  New seeds: {NEW_SEEDS}")
    log(f"  Algorithms: FedAvg, Ditto, HPFL, FedLESAM")
    log(f"  Datasets: chest_xray, Brain_Tumor, Skin_Cancer")
    ...
    for exp_idx, (ds_name, data_subdir, algo, seed, num_classes, short) in enumerate(experiments, 1):
        ...
        data_dir = str(FRAMEWORK_DIR / "data" / data_subdir)
        ...
        try:
            ...
            # Save even on error to not lose progress
            save_checkpoint(checkpoint_data)
    ...
    # Summary table
    log(f"\n  {'Dataset':<14} {'Algorithm':<12} {'Seed':<6} {'Best Acc':>10} {'Jain':>8} {'Rounds':>8}")
    log(f"  {'-'*62}")
    for ds_name, data_subdir, algo, seed, num_classes, short in experiments:
        key = f"{ds_name}_{algo}_s{seed}"
        r = checkpoint_data["completed"].get(key, {})
        if "error" in r:
            log(f"  {ds_name:<14} {algo:<12} {seed:<6} {'ERROR':>10}")
        elif r:
            acc = r.get("best_metrics", {}).get("accuracy", 0)
            jain = r.get("fairness", {}).get("jain_index", 0)
            rnds = r.get("actual_rounds", 0)
            log(f"  {ds_name:<14} {algo:<12} {seed:<6} {acc:>9.1%} {jain:>8.3f} {rnds:>8}")
