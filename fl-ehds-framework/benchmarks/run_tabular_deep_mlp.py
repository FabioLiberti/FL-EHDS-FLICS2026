#!/usr/bin/env python3
"""
FL-EHDS Tabular — Deep MLP Experiment.

Tests whether a deeper model (~110K params, 4-layer MLP) enables algorithm
differentiation that the shallow HealthcareMLP (~10K, near-convex) suppresses.

With the shallow model, 5/7 algorithms collapse to FedAvg-equivalent because
the loss landscape is nearly convex. This experiment uses a deeper model to
create a more non-convex landscape where server-side strategies (SCAFFOLD-style
variance reduction, FedExP acceleration, FedLESAM sharpness-awareness) can
potentially demonstrate their theoretical advantages.

Model: DeepHealthcareMLP — 4 layers [256, 256, 128, 64], ReLU, Dropout 0.3
       ~110K parameters (vs ~10K for HealthcareMLP)

Algorithms:  All 7 from the main evaluation
Datasets:    PTB-XL (5-class, 52 EU sites), Cardiovascular (binary, 70K)
Seeds:       5 seeds (42, 123, 456, 789, 1024)

Total: 7 algos × 2 datasets × 5 seeds = 70 experiments
...
Usage:
    ...
    python -m benchmarks.run_tabular_deep_mlp [--quick] [--fresh]

Output: benchmarks/paper_results_tabular/checkpoint_deep_mlp.json
...
class DeepHealthcareMLP(nn.Module):
    ...
    from FedAvg — unlike the shallow model where 5/7 algorithms collapse
...
# Same hyperparameters as shallow-model experiments, with more rounds
# for the deeper model to converge
DATASET_CONFIGS = {
    "PTB_XL": {
        "loader": "ptbxl",
        "input_dim": 9,
        ...
        "early_stopping": dict(enabled=True, patience=8, min_delta=0.002, min_rounds=15, metric="accuracy"),
    },
    "Cardiovascular": {
        ...
        "partition": dict(alpha=0.5),
        "early_stopping": dict(enabled=True, patience=8, min_delta=0.003, min_rounds=12, metric="accuracy"),
    },
}
...
def save_checkpoint(data: Dict) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(dir=str(OUTPUT_DIR), prefix=".ckpt_deep_", suffix=".tmp")
    try:
...
class EarlyStoppingMonitor:
    def __init__(self, patience=8, min_delta=0.002, min_rounds=15, metric="accuracy"):
...
def run_single(
    ...
    for r in range(num_rounds):
        ...
        log(f"[{exp_idx}/{total_exps}] {ds_config['short']} | {algorithm} | "
            f"s{seed} | R{r+1}/{num_rounds} | "
            f"Acc:{rr.global_acc:.1%} | Best:{best_acc:.1%}(r{best_round})")
    ...
    result = {
        ...
        "model_params": sum(p.numel() for p in trainer.global_model.parameters()),
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-EHDS — Deep MLP Algorithm Differentiation Experiment"
    )
    ...
    # Build experiment list: dataset × algo × seed
    experiments = []
    ...
    for ds_name, cfg in DATASET_CONFIGS.items():
        n_params = _count_params(cfg["input_dim"], cfg["num_classes"])
        log(f"  {ds_name}: {n_params:,} params")
    ...
    def _signal_handler(signum, frame):
        ...
        print(f"  Checkpoint salvato. Per riprendere: python -m benchmarks.run_tabular_deep_mlp")
        sys.exit(0)
    ...
    log(f"\n{'='*60}")
    log(f"  FL-EHDS Deep MLP Experiment ({mode})")
    log(f"  {total_exps} experiments = {len(ALGORITHMS)} algos × "
        f"{len(DATASET_CONFIGS)} DS × {len(seeds)} seeds")
    log(f"  Model: DeepHealthcareMLP [256, 256, 128, 64] (~110K params)")
    log(f"{'='*60}")
    ...
    log("\n  Deep MLP Results Summary:")
    log(f"  {'DS':>4} | {'Algorithm':<10} | {'Acc (%)':>10} | {'F1':>6} | {'Jain':>6}")
    log(f"  {'-'*4}-+-{'-'*10}-+-{'-'*10}-+-{'-'*6}-+-{'-'*6}")
    ...
    for ds_name, ds_config in DATASET_CONFIGS.items():
        ...
        for algo in ALGORITHMS:
            ...
            for key, res in completed.items():
                if ("error" not in res
                        and res.get("dataset") == ds_name
                        and res.get("algorithm") == algo):
                    accs.append(res.get("best_metrics", {}).get("accuracy", 0))
                    fm = res.get("final_metrics", {})
                    f1s.append(fm.get("f1", 0))
                    jains.append(res.get("fairness", {}).get("jain_index", 0))
            if accs:
                acc_str = f"{100*np.mean(accs):5.1f}±{100*np.std(accs):4.1f}"
                f1_str = f"{np.mean(f1s):.3f}" if f1s else "  --  "
                jain_str = f"{np.mean(jains):.3f}" if jains else "  --  "
                log(f"  {short:>4} | {algo:<10} | {acc_str:>10} | {f1_str:>6} | {jain_str:>6}")
            else:
                log(f"  {short:>4} | {algo:<10} |        --  |    --  |    --")

    # Compare with shallow MLP (if available)
    shallow_path = OUTPUT_DIR / "checkpoint_tabular.json"
    if shallow_path.exists():
        log("\n  Shallow vs Deep MLP Comparison:")
        try:
            with open(shallow_path) as f:
                shallow_data = json.load(f)
            shallow_completed = shallow_data.get("completed", {})
            log(f"  {'DS':>4} | {'Algorithm':<10} | {'Shallow':>8} | {'Deep':>8} | {'Delta':>6}")
            log(f"  {'-'*4}-+-{'-'*10}-+-{'-'*8}-+-{'-'*8}-+-{'-'*6}")
            for ds_name, ds_config in DATASET_CONFIGS.items():
                short = ds_config["short"]
                for algo in ALGORITHMS:
                    # Deep results
                    deep_accs = [res["best_metrics"]["accuracy"]
                                 for key, res in completed.items()
                                 if "error" not in res and res.get("dataset") == ds_name
                                 and res.get("algorithm") == algo]
                    # Shallow results
                    shallow_accs = [res["best_metrics"]["accuracy"]
                                    for key, res in shallow_completed.items()
                                    if "error" not in res and res.get("dataset") == ds_name
                                    and res.get("algorithm") == algo
                                    and "best_metrics" in res]
                    if deep_accs and shallow_accs:
                        d = 100 * np.mean(deep_accs)
                        s = 100 * np.mean(shallow_accs)
                        delta = d - s
                        log(f"  {short:>4} | {algo:<10} | {s:6.1f}%  | {d:6.1f}%  | {delta:+5.1f}pp")
        except Exception as e:
            log(f"  (Could not load shallow results: {e})")
