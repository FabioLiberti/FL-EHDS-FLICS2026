...
To validate the framework beyond synthetic tabular data, we conduct federated experiments on the Chest X-ray pneumonia detection dataset~\cite{chestxray2018} (5,856 pediatric radiographs, 2 classes) using HealthcareCNN, a 5-block CNN with GroupNorm ($\sim$12M parameters). Full experimental details are provided in Appendix~F.

\begin{table}[htbp]
\centering
\caption{FL Results on Chest X-ray Pneumonia Detection (5 hospitals, 20 rounds, mean $\pm$ std over 3 seeds)}
\label{tab:imaging_main}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Acc.} & \textbf{F1} & \textbf{AUC} & \textbf{Rec.} \\
\midrule
FedAvg (IID) & 87.3\%$\pm$1.2 & 0.89$\pm$0.01 & 0.85$\pm$0.02 & 0.92 \\
FedAvg (Non-IID) & 84.8\%$\pm$2.1 & 0.87$\pm$0.02 & 0.82$\pm$0.03 & 0.91 \\
FedProx (Non-IID) & 85.4\%$\pm$1.8 & 0.88$\pm$0.01 & 0.83$\pm$0.02 & 0.91 \\
FedAvg + DP ($\varepsilon$=10) & 81.2\%$\pm$2.4 & 0.84$\pm$0.03 & 0.78$\pm$0.04 & 0.89 \\
FedAvg + DP ($\varepsilon$=1) & 76.5\%$\pm$3.1 & 0.80$\pm$0.04 & 0.73$\pm$0.05 & 0.86 \\
\bottomrule
\end{tabular}
\end{table}

Clinical imaging results demonstrate substantially higher performance than synthetic tabular data: FedAvg achieves 87.3\% accuracy on IID data, with only 2.5pp Non-IID degradation. Differential privacy at $\varepsilon$=10 incurs a 3.6pp cost---higher than tabular but clinically viable for screening. Recall remains above 86\% in all configurations, appropriate for pneumonia screening where false negatives are costly.
...
\begin{itemize}
    \item \textbf{Feature extraction}: 5 convolutional blocks with GroupNorm, ReLU, MaxPool, and Dropout (0.15--0.3)
