...
A modular Python implementation of the FL-EHDS framework is available as open-source software\footnote{Available at: \url{https://github.com/FabioLiberti/FL-EHDS-FLICS2026}}. The implementation provides: (1) governance components for HDAB integration, permit management, and Article 71 opt-out compliance; (2) orchestration modules implementing FedAvg/FedProx aggregation with differential privacy ($\varepsilon$-budget tracking) and secure aggregation; (3) data holder utilities for adaptive training and FHIR R4 preprocessing.
...
We evaluate the FL-EHDS framework through controlled experiments simulating cross-border healthcare analytics. All experiments use synthetic cardiovascular risk prediction data to ensure reproducibility while maintaining clinical relevance.

\subsection{Experimental Setup}

\textbf{Task}: Binary classification for cardiovascular event risk prediction using five clinical features: age, BMI, systolic blood pressure, glucose level, and cholesterol. Features are normalized and labels are generated from a logistic risk model.

\textbf{Data Distribution}: We simulate 5 hospitals across EU Member States with heterogeneous data distributions (non-IID). Each hospital has 300--500 patient records with hospital-specific demographic biases reflecting real-world population differences (e.g., age distributions varying by region).

\textbf{Model}: Logistic regression classifier trained via federated optimization. While simpler than deep learning models used in production systems, this choice isolates FL algorithm behavior from model complexity confounds.

\textbf{Configurations}: We evaluate FedAvg~\cite{mcmahan2017communication} and FedProx~\cite{li2020federated} under varying conditions: IID vs.\ non-IID data, and differential privacy budgets $\varepsilon \in \{1, 10, \infty\}$.

\subsection{Results}

Table~\ref{tab:experimental_results} summarizes experimental outcomes across 30 training rounds with 5 participating hospitals.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Accuracy} & \textbf{Loss} & \textbf{$\varepsilon$} \\
\midrule
...
\footnotesize{5 hospitals, 30 rounds, 3 local epochs, batch size 32. Gradient clipping $C$=1.0.}
\end{table}

\textbf{Key Observations}:

\begin{enumerate}
    \item \textbf{Non-IID Impact}: Contrary to some literature findings, the non-IID configuration achieved slightly higher accuracy (58.4\% vs.\ 57.0\%). This suggests that for low-dimensional problems with sufficient data diversity, non-IID distributions may provide regularization benefits.

    \item \textbf{FedProx Performance}: FedProx with proximal term $\mu$=0.1 achieved comparable accuracy to FedAvg (58.0\% vs.\ 58.4\%), with reduced variance across client models (std.\ dev.\ 0.023 vs.\ 0.025).

    \item \textbf{Privacy-Utility Tradeoff}: Differential privacy introduces measurable accuracy degradation: 4.8 percentage points for $\varepsilon$=10, increasing to 5.6 percentage points for strong privacy ($\varepsilon$=1). This quantifies the cost of formal privacy guarantees.

    \item \textbf{Client Heterogeneity}: Per-hospital accuracy ranged from 57.7\% to 64.3\%, reflecting the non-IID data distributions. The global model generalizes across hospitals despite local distribution shifts.
\end{enumerate}

\subsection{Privacy Budget Analysis}

Figure~\ref{fig:privacy_tradeoff} illustrates the privacy-utility tradeoff. With strong privacy guarantees ($\varepsilon$=1), accuracy degrades by approximately 10\% relative to baseline, consistent with theoretical expectations for Gaussian mechanism noise~\cite{dwork2014dp}.
...
\addplot[fill=blue!60] coordinates {($\infty$,58.4) (10,53.6) (1,52.8)};
\end{axis}
\end{tikzpicture}
\caption{Privacy-utility tradeoff: accuracy vs.\ differential privacy budget $\varepsilon$.}
...
% ============================================================================
% APPENDIX - PSEUDOCODE (Outside page count)
% ============================================================================
\appendix
...
\fbox{\parbox{0.92\columnwidth}{
\small
\textbf{Algorithm 4: Secure Aggregation (Simplified)}\\[2pt]
...
\textbf{for each} client $k$ \textbf{do}\\
\hspace*{4mm}$\hat{\Delta}_k \leftarrow \Delta_k + \sum_{j<k} r_{jk} - \sum_{j>k} r_{kj}$\\
\hspace*{4mm}// where $r_{ij}$ are pairwise random masks\\[2pt]
// Phase 3: Reconstruction\\
$\Delta_{agg} \leftarrow \sum_{k=1}^{K} \hat{\Delta}_k$\\
// Masks cancel out: $\sum r_{jk} - \sum r_{kj} = 0$\\[2pt]
