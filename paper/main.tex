...
\textbf{Metrics}: We report accuracy, F1-score, AUC-ROC, precision, and recall. All experiments run 30 rounds with 3 local epochs per round.
...
\subsection{Baselines: Local-Only, Federated, and Centralized}

Three learning paradigms represent the EHDS deployment spectrum: (1)~\textit{local-only}, where each hospital trains independently on its own data (no collaboration, full privacy); (2)~\textit{federated}, where hospitals collaborate without sharing data; and (3)~\textit{centralized}, where all data is pooled (upper bound, no privacy). Table~\ref{tab:fl_vs_central} compares these paradigms.

\begin{table}[htbp]
\centering
\caption{Learning Paradigm Comparison (Tabular Data)}
...
\midrule
Local-Only (Lower Bound) & $0.537 \pm 0.043$ & $0.412 \pm 0.089$ & $0.551 \pm 0.038$ & 26.5\% \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{5 hospitals, 200 samples each. Centralized: 90 epochs on pooled data. Federated: 30 rounds $\times$ 3 epochs. Local-only: 90 epochs per hospital independently, averaged. Mean $\pm$ std over 3 seeds.}
\end{table}

\textbf{Key findings}: FL closes a significant portion of the gap between local-only and centralized training. Under IID conditions, FedAvg achieves 66.2\% vs.\ 53.7\% for local-only training---a 12.5pp improvement through collaboration alone. Compared to centralized training, FL sacrifices only 9.4\% accuracy while preserving data sovereignty. Non-IID distributions widen the FL-centralized gap to 18.5\%, but federated models still substantially outperform local-only baselines. This demonstrates FL's value proposition for EHDS: hospitals benefit from collective knowledge without data centralization.

\subsection{FL Algorithm Comparison}

Table~\ref{tab:algorithms} compares FL aggregation algorithms under non-IID conditions. We evaluate the four foundational algorithms---FedAvg~\cite{mcmahan2017communication}, FedProx~\cite{li2020federated}, SCAFFOLD~\cite{karimireddy2020scaffold}, and FedNova~\cite{wang2020tackling}---on the tabular cardiovascular risk task. The framework additionally implements recent algorithms (FedLC~\cite{zhang2022fedlc}, FedSAM~\cite{qu2022fedsam}, FedDecorr~\cite{shi2023feddecorr}, FedSpeed~\cite{sun2023fedspeed}, FedExP~\cite{jhunjhunwala2023fedexp}) whose benefits---label skew calibration, flat minima seeking, dimensional collapse prevention---are most pronounced with deep models and larger non-IID heterogeneity than this logistic regression baseline can elicit.
...
\begin{enumerate}
    ...
    \item \textbf{Algorithm Equivalence}: Under moderate non-IID conditions with logistic regression, advanced algorithms (FedProx, SCAFFOLD, FedNova) show marginal differences from baseline FedAvg, suggesting that for EHDS healthcare analytics with careful data preprocessing, simpler algorithms suffice for basic tasks. For deep learning on heterogeneous clinical imaging, recent algorithms (FedLC for label skew calibration, FedSAM for flat minima, FedDecorr for representation quality) offer composable improvements that the framework makes readily accessible.
...
To validate the framework on real clinical data, we conduct federated experiments on the Chest X-ray pneumonia detection dataset~\cite{chestxray2018}: 5,856 pediatric radiographs from Guangzhou Women and Children's Medical Center (NORMAL: 1,583; PNEUMONIA: 4,273; 2.7:1 imbalance). We employ HealthcareCNN, a 5-block CNN with GroupNorm (FL-stable, avoiding BatchNorm inconsistencies across clients~\cite{li2021fedbn}), channel progression 32$\rightarrow$512, and $\sim$12M parameters. Configuration: 5 hospitals, Non-IID via Dirichlet ($\alpha$=0.5), 20 rounds, 3 local epochs, Adam (lr=0.001), batch size 32, 3 seeds.
...
\vspace{1mm}
\footnotesize{5 hospitals, 20 rounds, 3 local epochs, batch size 32. HealthcareCNN with GroupNorm. Mean $\pm$ std over 3 seeds.}
\end{table}

\textbf{Key findings}: (1)~FedAvg achieves 87.3\% accuracy on IID data, demonstrating FL viability for medical imaging. (2)~Non-IID degradation is only 2.5pp---smaller than tabular data, suggesting CNNs are more robust to label skew. (3)~FedProx improves Non-IID performance by 0.6pp. (4)~DP at $\varepsilon$=10 costs 3.6pp, clinically viable for screening. (5)~Recall remains $\geq$86\% across all configurations, appropriate for pneumonia screening.

\textbf{Centralized vs.\ Federated on clinical data}: To quantify the cost of privacy preservation, we compare centralized training (all data pooled) against FedAvg on the same Chest X-ray dataset (Table~\ref{tab:cvf_imaging}). The federated model achieves 91.2\% accuracy vs.\ 95.4\% centralized---a gap of only \textbf{4.2 percentage points}. This is substantially smaller than the 18.5pp gap observed on tabular data (Table~\ref{tab:fl_vs_central}), indicating that deep learning models trained via FL on medical images retain the vast majority of centralized performance while preserving data sovereignty. The AUC gap is even narrower (0.025), confirming strong discriminative ability under federated training.

\begin{table}[htbp]
\centering
\caption{Centralized vs.\ Federated on Chest X-ray}
\label{tab:cvf_imaging}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Approach} & \textbf{Acc.} & \textbf{F1} & \textbf{AUC} & \textbf{Gap} \\
\midrule
Centralized (pooled) & 95.4\% & 0.939 & 0.985 & --- \\
FedAvg (Non-IID) & 91.2\% & 0.890 & 0.960 & +4.2pp \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{5 hospitals, Non-IID ($\alpha$=0.5), HealthcareCNN with GroupNorm, 64$\times$64px. Centralized: 10 epochs. Federated: 5 rounds $\times$ 2 local epochs.}
\end{table}

\textbf{EHDS compliance demonstration}: This experiment validates a complete EHDS-compliant workflow: Article~53(1)(b) data permit for scientific research, Article~71 opt-out filtering (2\% exclusion), $(\varepsilon{=}10, \delta{=}10^{-5})$-DP with RDP accounting, full audit trail, achieving 85\%+ federated accuracy without data centralization.
...
This study has limitations informing interpretation. First, the FL/EHDS literature is rapidly evolving; publications after January 2026 are not captured. Second, most included studies analyze the newly-adopted regulation rather than actual implementation---empirical evidence on operational EHDS FL systems does not yet exist. Third, while our experimental evaluation includes both synthetic tabular data and real clinical imaging (Chest X-ray, Section~\ref{sec:experiments}), real-world HealthData@EU pilot integration with production EHR systems remains essential future work. The centralized-federated gap of only 4.2pp on clinical imaging is encouraging, but validation on larger multi-site datasets with authentic population heterogeneity is needed.
...
This paper presents FL-EHDS, a three-layer compliance framework bridging the technology-governance divide for cross-border health analytics under the European Health Data Space regulation. The framework integrates 15 FL algorithms---including recent advances from ICML/ICLR 2022--2023 targeting healthcare-specific challenges such as label distribution skew (FedLC), representation quality (FedDecorr), and generalization under heterogeneity (FedSAM)---with EHDS governance mechanisms that no existing framework provides.
