...
FL-EHDS uniquely bridges these dimensions by: (1) grounding the framework in systematic evidence synthesis; (2) explicitly addressing EHDS regulatory requirements; (3) mapping technical barriers to governance-aware mitigation strategies; and (4) incorporating recent FL advances from top venues (ICML/ICLR 2022--2025)---including FedLC~\cite{zhang2022fedlc} for label distribution skew, FedSAM~\cite{qu2022fedsam} for sharpness-aware generalization, FedDecorr~\cite{shi2023feddecorr} for representation quality, FedSpeed~\cite{sun2023fedspeed} for communication efficiency, FedExP~\cite{jhunjhunwala2023fedexp} for server-side convergence acceleration, FedLESAM~\cite{qu2024fedlesam} for globally-guided sharpness awareness, and HPFL~\cite{chen2025hpfl} for personalized classifier federation---that directly address healthcare-specific data heterogeneity challenges. FL-EHDS's governance layer could be integrated as a Flower strategy wrapper, enabling EHDS compliance within the Flower ecosystem.
...
\textbf{Aggregation Module}: The framework implements 17 aggregation algorithms spanning six categories. \textit{Baseline}: FedAvg~\cite{mcmahan2017communication}. \textit{Non-IID robustness}: FedProx~\cite{li2020federated} (proximal regularization), SCAFFOLD~\cite{karimireddy2020scaffold} (control variates for variance reduction), FedNova~\cite{wang2020tackling} (normalized averaging under heterogeneous local steps), FedDyn (dynamic regularization). \textit{Adaptive server optimization}: FedAdam, FedYogi, FedAdagrad~\cite{reddi2021adaptive}. \textit{Personalization}: Ditto (fair and robust FL with dual models), Per-FedAvg (MAML-based fine-tuning). \textit{Recent advances (2022--2023)}: FedLC~\cite{zhang2022fedlc} calibrates logits via class-frequency margins, directly addressing label distribution skew prevalent when hospitals have different disease prevalences; FedSAM~\cite{qu2022fedsam} applies Sharpness-Aware Minimization locally to seek flat minima; FedDecorr~\cite{shi2023feddecorr} adds a decorrelation regularizer to prevent dimensional collapse; FedSpeed~\cite{sun2023fedspeed} unifies proximal correction with gradient perturbation; FedExP~\cite{jhunjhunwala2023fedexp} computes an adaptive server step size inspired by POCS, requiring zero client-side modifications. \textit{Latest advances (2024--2025)}: FedLESAM~\cite{qu2024fedlesam} extends FedSAM by replacing local gradient perturbation with a globally-estimated direction ($\theta^{(t-1)}_{\text{global}} - \theta^{(t)}_{\text{global}}$), achieving stronger generalization across heterogeneous institutions (ICML 2024 Spotlight); HPFL~\cite{chen2025hpfl} decouples feature extraction from classification by aggregating only backbone parameters while keeping client-specific classifier heads local, enabling each hospital to maintain specialized decision boundaries adapted to its patient population (ICLR 2025). Algorithm selection is configurable based on deployment requirements; FedLC and FedDecorr are composable with any aggregation strategy. Table~\ref{tab:algo_catalogue} summarizes all 17 algorithms with their key properties.
...
\vspace{1mm}
\footnotesize{\textbf{Bold}: newly added algorithms (2024--2025). All algorithms are implemented in the open-source reference implementation. Extended descriptions and pseudocode in Appendix~\ref{appendix:pseudocode} and~\ref{appendix:advanced_fl}.}
\end{table}
...
\noindent The implementation provides: (1) orchestration modules implementing 17 FL algorithms---from foundational methods (FedAvg, FedProx, SCAFFOLD, FedNova) through adaptive server optimizers (FedAdam, FedYogi, FedAdagrad) and personalization methods (Ditto, Per-FedAvg) to recent advances addressing healthcare-specific challenges (FedLC, FedSAM, FedDecorr, FedSpeed, FedExP, FedLESAM, HPFL)---with R\'enyi differential privacy accounting (validated experimentally in Section~\ref{sec:experiments}) and secure aggregation; (2) six Byzantine resilience methods (Krum, Multi-Krum, Trimmed Mean, Median, Bulyan, FLTrust/FLAME); (3) data holder utilities for adaptive training and FHIR R4 preprocessing; (4) reproducible benchmarks generating all experimental results.
...
\textbf{Configuration}: 20 rounds, 3 local epochs, batch size 32, Adam optimizer (lr=0.01 tabular, 0.001 imaging). Non-IID via natural hospital partitioning (Heart Disease) or Dirichlet $\alpha$=0.5 (Diabetes). All results are mean $\pm$ std over 3 seeds. Figure~\ref{fig:stat_heterogeneity} visualizes the statistical heterogeneity across participating hospitals.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/fig_statistical_heterogeneity.pdf}
\caption{Statistical heterogeneity across hospital sites. Non-IID conditions arise from demographic differences, disease prevalence, and clinical protocols---representative of real EHDS cross-border federation scenarios.}
\label{fig:stat_heterogeneity}
\end{figure}
...
The framework integrates differential privacy (DP) via Opacus-based per-sample gradient clipping and Gaussian noise injection, with R\'{e}nyi Differential Privacy (RDP) accounting~\cite{mironov2017renyi}. Table~\ref{tab:privacy_utility} reports the privacy-utility tradeoff on the imaging task, where the DP overhead is most informative due to higher model capacity.

\begin{table}[htbp]
\centering
\caption{Privacy-Utility Tradeoff (Imaging, FedAvg + DP)}
\label{tab:privacy_utility}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Privacy ($\varepsilon$)} & \textbf{Accuracy} & \textbf{$\Delta$ vs.\ No DP} & \textbf{Guarantee} \\
\midrule
No DP ($\varepsilon{=}\infty$) & Baseline & --- & None \\
$\varepsilon{=}10$ & Baseline $-$5.2pp & $-$5.2pp & Moderate \\
$\varepsilon{=}1$ & Baseline $-$5.8pp & $-$5.8pp & Strong \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{RDP accounting with $\delta{=}10^{-5}$, gradient clipping $C{=}1.0$, Gaussian noise. The 5--6pp accuracy cost provides formal $(\varepsilon,\delta)$-DP guarantees satisfying EHDS Article~50 SPE requirements.}
\end{table}
...
Figure~\ref{fig:noniid_impact} illustrates the impact of data heterogeneity on algorithm performance, showing that algorithm selection becomes increasingly critical as non-IID severity grows.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/fig_accuracy_vs_noniid.pdf}
\caption{Final accuracy vs.\ data heterogeneity level (Dirichlet $\alpha$). As non-IID severity increases ($\alpha \to 0$), algorithm choice becomes critical: variance-reduction methods (SCAFFOLD) maintain stability while baseline FedAvg degrades. See Appendix~\ref{appendix:algorithm_comparison} for extended analysis.}
\label{fig:noniid_impact}
\end{figure}

\subsection{Algorithm Analysis}

Beyond the five algorithms benchmarked in Table~\ref{tab:main_results}, the framework implements recent methods spanning 2022--2025: FedLC~\cite{zhang2022fedlc}, FedSAM~\cite{qu2022fedsam}, FedDecorr~\cite{shi2023feddecorr}, FedSpeed~\cite{sun2023fedspeed}, FedExP~\cite{jhunjhunwala2023fedexp}, FedLESAM~\cite{qu2024fedlesam}, and HPFL~\cite{chen2025hpfl}. FedLESAM extends FedSAM with globally-estimated perturbation directions, providing stronger generalization guarantees; HPFL keeps classifier heads local per client while aggregating only the shared backbone, enabling each hospital to maintain specialized decision boundaries. These methods---together with label skew calibration (FedLC), dimensional collapse prevention (FedDecorr), and communication-efficient training (FedSpeed, FedExP)---are most pronounced with deep models on imaging tasks. Extended experimental evaluation across 7 algorithms, 5 datasets, and 3 seeds is reported in Appendix~\ref{appendix:imaging}. On tabular data, \textbf{Ditto} emerges as the clear winner: its personalized local models outperform FedAvg by 12.6pp on Heart Disease and 3.6pp on Diabetes. SCAFFOLD and FedNova diverge on Diabetes ($\leq$13\% accuracy), likely due to the severe 11\% positive rate creating pathological gradient updates under variance reduction.
...
\begin{enumerate}
    ...
    \item \textbf{Recent algorithms add value}: FedLESAM~\cite{qu2024fedlesam} (2024) and HPFL~\cite{chen2025hpfl} (2025) represent the latest FL research specifically addressing healthcare heterogeneity. FedLESAM's globally-guided perturbation improves generalization under cross-border distribution shifts; HPFL's local classifiers enable per-hospital specialization while maintaining shared feature learning. Extended evaluation across 7 algorithms and 5 datasets in Appendix~\ref{appendix:imaging}.
\end{enumerate}

\subsection{Clinical Imaging Validation}
\label{subsec:imaging}

To validate the framework beyond tabular data, we conduct federated experiments on three clinical imaging datasets: (1)~\textit{Chest X-ray} pneumonia detection~\cite{chestxray2018} (5,860 pediatric radiographs, 2.7:1 class imbalance), (2)~\textit{Brain Tumor MRI} (3-class glioma/meningioma/pituitary classification), and (3)~\textit{Skin Cancer} dermoscopy (benign/malignant classification). We employ ResNet-18~\cite{he2016deep} with GroupNorm (FL-stable, avoiding BatchNorm inconsistencies across clients~\cite{li2021fedbn}), partial backbone freezing (first convolutional block), and FedBN~\cite{li2021fedbn} for normalization-layer-aware aggregation. Configuration: 5 hospitals, Non-IID via Dirichlet ($\alpha$=0.5), 25 rounds, 3 local epochs, Adam (lr=0.001), batch size 32, 3 seeds. Seven representative algorithms are evaluated including two recent methods (2024--2025).

\begin{table}[htbp]
\centering
\caption{FL Algorithm Comparison on Clinical Imaging Datasets}
\label{tab:imaging_main}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Algorithm} & \textbf{Chest X-ray} & \textbf{Brain Tumor} & \textbf{Skin Cancer} \\
...
\vspace{1mm}
\footnotesize{5 hospitals, Non-IID ($\alpha$=0.5), ResNet-18 with GroupNorm and FedBN. 25 rounds, 3 local epochs. Mean $\pm$ std over 3 seeds. Full results including F1, AUC, and per-client analysis in Appendix~\ref{appendix:imaging}. Communication: 44.7~MB/round per client (reducible to 0.45~MB with Top-$k$ 1\%).}
\end{table}

The imaging pipeline demonstrates the framework's modular design: the same FL orchestrator, DP module, and EHDS governance layer used for tabular experiments seamlessly extend to deep learning on medical images. The key architectural choice---GroupNorm over BatchNorm---ensures consistent normalization statistics across heterogeneous hospital clients, a known issue in federated imaging~\cite{li2021fedbn}. Figure~\ref{fig:label_skew} illustrates the non-IID label distribution across simulated hospital sites, showing significant class imbalance variation.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/fig_label_skew_analysis.pdf}
\caption{Label distribution skew across hospital sites (Dirichlet $\alpha$=0.5). Each hospital receives a different proportion of disease classes, creating realistic non-IID conditions. FedLC~\cite{zhang2022fedlc} and HPFL~\cite{chen2025hpfl} are specifically designed to address this challenge.}
\label{fig:label_skew}
\end{figure}
...
This paper presents FL-EHDS, a three-layer compliance framework bridging the technology-governance divide for cross-border health analytics under the European Health Data Space regulation. The framework integrates 17 FL algorithms---including recent advances from ICML/ICLR 2022--2025 targeting healthcare-specific challenges such as label distribution skew (FedLC), representation quality (FedDecorr), generalization under heterogeneity (FedSAM, FedLESAM~\cite{qu2024fedlesam}), and personalized federation with shared backbone (HPFL~\cite{chen2025hpfl})---with EHDS governance mechanisms that no existing framework provides. Experimental validation on real clinical datasets (Heart Disease UCI across 4 international hospitals, Diabetes 130-US with 101K encounters, medical imaging on Chest X-ray, Brain Tumor MRI, and Skin Cancer dermoscopy) demonstrates that personalized FL methods achieve the strongest privacy-utility tradeoff, while algorithm choice produces up to 18.7pp performance differences---underscoring the importance of algorithm selection for heterogeneous clinical data.
...
K. Wei, J. Li, M. Ding, C. Ma, H. H. Yang, F. Farokhi, S. Jin, T. Q. S. Quek, and H. V. Poor, ``Federated learning with differential privacy: Algorithms and performance analysis,'' \textit{IEEE Trans.~Inf.~Forensics Secur.}, vol.~15, pp.~3454--3469, 2020.

% === Deep Learning Architecture ===
\bibitem{he2016deep}
K. He, X. Zhang, S. Ren, and J. Sun, ``Deep residual learning for image recognition,'' in \textit{Proc. IEEE CVPR}, pp.~770--778, 2016.

% === Recent FL Algorithms (2024-2025) ===
\bibitem{qu2024fedlesam}
Z. Qu, X. Li, Y. Liu, R. Duan, and Z. Lu, ``FedLESAM: Federated learning with locally estimated sharpness-aware minimization,'' in \textit{Proc. ICML}, PMLR 235, 2024. (Spotlight)
...
\noindent\textit{Recent algorithms (2022--2025)} extending the framework beyond foundational methods:
\begin{itemize}
    ...
    \item \textbf{FedLESAM}~\cite{qu2024fedlesam}: Extends FedSAM with locally-estimated global perturbation direction ($\theta^{(t-1)}_g - \theta^{(t)}_g$); ICML 2024 Spotlight.
    \item \textbf{HPFL}~\cite{chen2025hpfl}: Hot-Pluggable FL---aggregates shared backbone only, keeps classifier heads local per client; ICLR 2025.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Chest X-ray}~\cite{chestxray2018}: 5,860 pediatric radiographs (NORMAL/PNEUMONIA, 2.7:1 imbalance). Binary classification for pneumonia detection.
    \item \textbf{Brain Tumor MRI}: 3,064 T1-weighted CE MRI slices. 3-class classification (glioma, meningioma, pituitary tumor).
    \item \textbf{Skin Cancer}: 3,297 dermoscopy images. Binary classification (benign vs.\ malignant).
\end{itemize}
...
\begin{itemize}
    \item \textbf{HealthcareResNet}: ResNet-18~\cite{he2016deep} pretrained on ImageNet, with GroupNorm replacing BatchNorm for FL stability. Partial backbone freezing (level 1: conv1+bn1 frozen). FedBN~\cite{li2021fedbn} skips normalization layers during aggregation. $\sim$11.2M parameters.
    \item \textbf{HealthcareCNN}: 5-block CNN with GroupNorm, progressive channels (32$\rightarrow$512), graduated Dropout (0.15$\rightarrow$0.3). Classifier: Flatten$\rightarrow$FC(512)$\rightarrow$FC(128)$\rightarrow$FC($K$). $\sim$12M parameters.
\end{itemize}

\noindent Data augmentation: random horizontal flip, rotation ($\pm$15\textdegree), brightness jitter ($\pm$10\%). ImageNet normalization.
...
\noindent Results, checkpoints, and logs are auto-saved to \texttt{benchmarks/paper\_results/}.
