...
Chest X-ray experiments use 4 algorithms (FedAvg, FedLESAM, Ditto, HPFL) $\times$ 3 seeds = 12 experiments per dataset. Brain Tumor and Skin Cancer use 2 algorithms (FedAvg, HPFL) $\times$ 1 seed as qualitative validation of the modality-dependent personalization effect; statistical significance is established on the tabular benchmarks (10 seeds, $p < 0.001$).
...
\begin{itemize}
    ...
    \item Brain Tumor / Skin Cancer: 10 rounds, early stopping (patience=3, min\_rounds=6), lr=0.0005/0.001
\end{itemize}
...
\vspace{1mm}
\footnotesize{TP: true positives (Malignant correctly identified) summed over 10 seeds (427 total Malignant samples). \textbf{Collapse}: seeds where the model predicts a \textit{single} class for all test samples (7/10 predict only Benign, 1/10 only Malignant for FedAvg/FedProx/Ditto). FedAvg, FedProx, and Ditto produce \textbf{nearly identical} predictions (algorithm collapse).}
\end{table}
...
Table~\ref{tab:imaging_bt_sc} extends the imaging evaluation to Brain Tumor MRI (4-class) and Skin Cancer (binary), providing qualitative validation of the modality-dependent personalization effect across three imaging datasets. Statistical significance is established on the tabular benchmarks (10 seeds, $p < 0.001$).

\begin{table}[htbp]
\centering
\caption{Brain Tumor MRI (4-class) and Skin Cancer (binary): FedAvg vs HPFL with ResNet-18. Single seed (42), 10 rounds, early stopping (patience=3). Best accuracy reported.}
\label{tab:imaging_bt_sc}
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Dataset} & \textbf{Algo} & \textbf{Acc (\%)} & \textbf{Jain} & \textbf{Rnds} \\
\midrule
\multirow{2}{*}{Brain Tumor} & FedAvg & 46.5 & 0.919 & 10 \\
 & HPFL & 45.6 & \textbf{0.937} & 9 \\
\midrule
\multirow{2}{*}{Skin Cancer} & FedAvg & 62.8 & 0.900 & 9 \\
 & HPFL & \textbf{68.5} & \textbf{0.987} & 10 \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{Single-seed results (seed=42). Brain Tumor uses lr=0.0005; Skin Cancer uses lr=0.001. Extended multi-seed evaluation is deferred to future work.}
\end{table}

\textit{Implications}: The three-dataset imaging evaluation reveals a \textbf{nuanced modality-dependent personalization effect}. On Chest X-ray (5,856 samples), FedAvg dominates (+18.2pp). On Skin Cancer (3,297 samples), HPFL outperforms FedAvg by +5.7pp with better fairness (Jain 0.987 vs.\ 0.900). On Brain Tumor (7,023 samples, 4-class), both achieve $\sim$46\%. The pattern suggests HPFL's personalized heads help on smaller, heterogeneous datasets but are counterproductive when the global model has sufficient data.
