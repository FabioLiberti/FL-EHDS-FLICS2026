...
\begin{itemize}
    \item \textit{Gradient inversion}~\cite{zhu2019deep}: reconstructing training samples from observed gradients. Effectiveness decreases with batch size and model depth; for the tabular MLP ($\sim$10K parameters, batch size 32--64), theoretical reconstruction quality is limited but non-zero.
...
\begin{table}[t]
\centering
\caption{Privacy-utility tradeoff: accuracy (\%) under central DP with varying $\varepsilon$. Gaussian mechanism, $C{=}1.0$, $\delta{=}10^{-5}$. Mean $\pm$ std over 5 seeds. No-DP baselines from Table~7 (main paper). $\Delta$ = drop from No-DP.}
...
\midrule
Ditto & Cardiovascular & 0.002 & 1.00 & +12.3 \\
 & Breast Cancer & 0.016 & 1.00 & +22.2 \\
 & Pooled (30 obs.) & $<$0.001 & 0.95 & +11.7 \\
\midrule
HPFL & PTB-XL & 0.004 & 1.00 & +1.1 \\
 & Cardiovascular & 0.002 & 1.00 & +12.3 \\
 & Breast Cancer & 0.031 & 0.93 & +19.5 \\
 & Pooled (30 obs.) & $<$0.001 & 0.97 & +10.9 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key findings}: With 10 seeds, the statistical picture sharpens decisively. \textit{First}, \textbf{HPFL significantly outperforms FedAvg on all three datasets} ($p = 0.004$, $0.002$, $0.031$; all $< 0.05$), with improvements of +1.1pp on PTB-XL, +12.3pp on Cardiovascular, and +19.5pp on Breast Cancer---the only algorithm achieving significance across all datasets. All effect sizes are large ($r \geq 0.93$; Table~\ref{tab:effect_sizes}). \textit{Second}, \textbf{Ditto significantly outperforms FedAvg on Cardiovascular and Breast Cancer} ($p = 0.002$, $0.016$), with large effect sizes ($r = 1.00$, $1.00$; $\overline{\Delta} = +12.3$pp, $+22.2$pp), though not on PTB-XL ($p = 0.08$, $\Delta = +0.5$pp) where FedAvg already achieves 91.5\%. \textit{Third}, \textbf{five algorithms collapse to FedAvg-equivalent performance}: FedLESAM produces identical results on all datasets; FedExP and FedProx are identical on 2/3 datasets. This confirms that only methods maintaining separate local models (Ditto, HPFL) differentiate on the near-convex tabular MLP landscape. \textit{Fourth}, the \textbf{pooled analysis} across all 30 paired observations yields $p < 0.001$ for both Ditto ($r = 0.95$) and HPFL ($r = 0.97$), providing strong evidence that personalized FL methods are superior for EHDS tabular analytics.
...
\begin{table}[t]
\centering
\caption{Article~71 opt-out impact: accuracy (\%) at different opt-out rates. $\Delta$: maximum drop relative to 0\% baseline. Mean $\pm$ std over 5 seeds.}
\label{tab:optout}
\footnotesize
...
\begin{table}[t]
\centering
\caption{Per-dataset optimized configuration used for baseline comparison (Table~7 in main paper).}
...
\begin{table}[htbp]
\centering
\caption{Deep MLP ($\sim$110K params) vs.\ Shallow MLP ($\sim$10K params). Accuracy (\%), mean $\pm$ std over 5 seeds.}
\label{tab:deep_mlp}
\small
\begin{tabular}{lcccc}
\toprule
...
\vspace{1mm}
\footnotesize{Shallow: HealthcareMLP [64, 32], $\sim$10K params. Deep: DeepHealthcareMLP [256, 256, 128, 64], $\sim$110K params. Same per-dataset hyperparameters (Table~\ref{tab:dataset_config}). Breast Cancer omitted: 569 samples insufficient for stable $\sim$110K-parameter training.}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Breast Cancer confusion matrix analysis: per-class recall and accuracy (\%). Mean over 3 seeds. Benign is the majority class ($\sim$63\% of test samples).}
\label{tab:confusion_matrix_bc}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Algorithm} & \textbf{Acc (\%)} & \textbf{Benign Recall} & \textbf{Malig.\ Recall} & \textbf{TP} \\
\midrule
FedAvg & 64.9{\tiny$\pm$2.3} & 1.000 & 0.000 & 0/119 \\
FedProx & 64.9{\tiny$\pm$2.3} & 1.000 & 0.000 & 0/119 \\
Ditto & 64.9{\tiny$\pm$2.3} & 1.000 & 0.000 & 0/119 \\
\midrule
HPFL & \textbf{95.9}{\tiny$\pm$3.3} & 0.977 & \textbf{0.918} & 110/119 \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{TP column: true positives (Malignant correctly identified) summed over 3 seeds (out of 119 total Malignant samples across seeds). FedAvg, FedProx, and Ditto produce \textbf{numerically identical} predictions (algorithm collapse).}
\end{table}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/confusion_matrix_bc.pdf}
\caption{Confusion matrices for Breast Cancer classification. FedAvg/FedProx/Ditto predict \textit{only} the majority class (Benign), achieving 0\% sensitivity on Malignant samples. HPFL is the sole algorithm that learns both classes (91.8\% Malignant recall).}
\label{fig:confusion_matrix_bc}
\end{figure}

\textbf{Key finding}: FedAvg, FedProx, and Ditto produce \textbf{pure majority-class predictors}---every test sample is classified as Benign, yielding zero true positives for the Malignant class. Their reported accuracy ($\sim$64.9\%) simply equals the class prior. This is the most extreme manifestation of the algorithm collapse: the federated optimization converges to the trivial solution of predicting the most frequent class, identical across all three algorithms. HPFL's personalized classifier heads escape this collapse, achieving 95.9\% accuracy with 91.8\% Malignant recall---correctly identifying 110 of 119 Malignant samples that every other algorithm misses. \textit{For EHDS deployment on class-imbalanced clinical tasks (e.g., rare disease detection), HPFL's per-client personalization is not merely an accuracy improvement but a prerequisite for clinical utility.}
...
Top-5\% & 78.7{\tiny$\pm$8.5} & $-$13.4 & 95\% & 144/2,885 \\
Top-1\% & 77.3{\tiny$\pm$8.6} & $-$14.7 & 99\% & 29/2,885 \\
\bottomrule
\end{tabular}
...
We verify that $\varepsilon{=}10$ DP remains ``free'' ($<$2pp accuracy cost) across algorithms and client counts on PTB-XL (Table~\ref{tab:dp_scale}). Three algorithms (FedAvg, Ditto, HPFL) are tested at $K{=}5$ with and without DP ($\varepsilon{=}10$, $C{=}1.0$, $\delta{=}10^{-5}$).
...
% Bibliography synchronized with main paper (51 entries)
...
% === Entries synchronized from main paper ===
