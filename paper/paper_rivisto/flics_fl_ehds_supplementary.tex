...
\subsection{Chest X-ray Baseline Results}
\label{sec:imaging_results}

Table~\ref{tab:imaging_chest} reports baseline Chest X-ray results for FedAvg and Ditto (ResNet-18, 5 clients, Dirichlet $\alpha{=}0.5$, 20 rounds max with early stopping).

\begin{table}[htbp]
\centering
\caption{Chest X-ray pneumonia detection: federated accuracy (\%) with ResNet-18. 5 clients, Dirichlet $\alpha{=}0.5$, 20 rounds, Adam lr=0.001, early stopping (patience=6). Per-seed results with per-client accuracy range.}
...
\vspace{1mm}
\footnotesize{Jain index averaged over 3 seeds. FedAvg achieves stable convergence across seeds (12--20 rounds); Ditto shows high seed sensitivity (s123: only 8 rounds, 65.1\%).}
\end{table}

\textbf{Key findings.} \textit{First}, federated learning on Chest X-ray achieves \textbf{87.3\% accuracy} (FedAvg) for binary pneumonia detection with ResNet-18, confirming FL viability for medical imaging under EHDS-relevant non-IID conditions. \textit{Second}, \textbf{personalization (Ditto) does not improve on imaging}: Ditto achieves 80.0\% vs.\ FedAvg 87.3\% ($-$7.3pp), with substantially higher variance across seeds ($\sigma = 10.6$pp vs.\ 3.6pp). This contrasts sharply with tabular results where Ditto consistently outperforms FedAvg by 12--22pp. The likely explanation is that ResNet-18's 11.2M-parameter backbone already provides sufficient representational capacity for the global model to generalize across clients; Ditto's dual-model overhead (maintaining separate local models) increases training instability without compensating benefit. \textit{Third}, Ditto exhibits a \textbf{fairness failure mode} on seed 456: per-client accuracies of [99.7\%, 100.0\%, 48.5\%, 39.5\%, 100.0\%] (Jain 0.888) indicate that personalized local models overfit to majority-class clients while failing on minority-class clients under Dirichlet partitioning. FedAvg's global model distributes performance more evenly.

\textit{Implications for EHDS}: The imaging results reveal a \textbf{modality-dependent personalization effect}: personalized FL is critical for tabular clinical analytics (where lightweight models produce near-convex landscapes that personalization escapes) but potentially counterproductive for deep imaging models (where the global model already exploits the non-convex landscape effectively). EHDS data permits should specify algorithm recommendations conditioned on the analytics modality, not as a blanket policy.
