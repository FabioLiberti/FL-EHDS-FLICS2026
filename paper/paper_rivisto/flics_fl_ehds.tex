...
\textbf{Communication \& imaging}: Tabular FL requires only 0.04~MB/round (10K-param MLP, 0.8~MB total). Imaging (ResNet-18, 11.2M params) requires 44.7~MB/round; Top-$k$ sparsification at 5\% achieves 95\% bandwidth savings albeit with accuracy cost on small models (Supplementary Material, Table~XXI). Chest X-ray validation~\cite{chestxray2018} with ResNet-18~\cite{he2016deep}, GroupNorm, and FedBN~\cite{li2021fedbn} achieves 87.3\% accuracy (FedAvg, 5 clients, Dirichlet $\alpha{=}0.5$). Crucially, \textbf{personalization is counterproductive on imaging}: Ditto achieves only 80.0\% ($-$7.3pp) and HPFL fails catastrophically at 69.1\% ($-$18.2pp), while FedLESAM matches FedAvg exactly (87.8\%). Confusion matrix analysis (3 seeds, Table~XX) confirms the mechanism: FedAvg achieves 95.5\% PNEUMONIA recall vs.\ HPFL's 72.4\%, with HPFL's per-client classifier heads overfitting local distributions (Client~3: 23.5\% PNEUMONIA recall). This reveals a \textbf{modality-dependent personalization effect}: lightweight tabular models benefit critically from personalization, but deep imaging models generalize better with global aggregation. On Breast Cancer, confusion matrix analysis (10 seeds) confirms this asymmetry from the opposite direction: FedAvg/FedProx/Ditto exhibit single-class collapse on 8/10 data partitions (21.5\% aggregated Malignant recall, with 0\% on 7 individual seeds), while HPFL avoids collapse on all seeds (78.7\% aggregated Malignant recall; Supplementary Material, Table~XIX). EHDS data permits should condition algorithm selection on analytics modality.
