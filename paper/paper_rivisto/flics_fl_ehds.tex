...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/arch_3layer.jpg}
\caption{FL-EHDS three-layer architecture. Layer~1 (Governance) manages HDAB integration across Member States, data permit authorization, opt-out registries (Art.~71), purpose limitation (Art.~53), and multi-HDAB synchronization for cross-border coordination. Layer~2 (FL Orchestration) operates within the Secure Processing Environment (SPE) with three modules: Aggregation (17 FL algorithms including FedAvg, FedProx, non-IID adaptation), Privacy Protection (differential privacy $\varepsilon$, secure aggregation, gradient clipping, MIA defense), and Compliance (GDPR Art.~30 audit logging, purpose limitation check, opt-out enforcement, anonymity assessment). Layer~3 (Data Holders) implements a uniform stack per institution---hospitals, disease registries, and research centers across 27 Member States---with FHIR-formatted data and local training engines. Only encrypted model gradients are exchanged within the SPE; raw health data never leaves institutional boundaries.}
...
\textbf{Communication \& imaging}: Tabular FL requires only 0.04~MB/round (10K-param MLP, 0.8~MB total). Imaging (ResNet-18, 11.2M params) requires 44.7~MB/round, reducible with Top-$k$ sparsification (1\%). The framework supports ResNet-18~\cite{he2016deep} with GroupNorm and FedBN~\cite{li2021fedbn} on Chest X-ray~\cite{chestxray2018}, Brain Tumor MRI, and Skin Cancer; preliminary imaging validation on Chest X-ray confirms that the personalization advantage persists in the imaging domain (comprehensive imaging evaluation across all algorithms and datasets is provided in Supplementary Material).
