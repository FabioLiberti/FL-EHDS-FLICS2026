...
\textbf{Communication \& imaging}: Tabular FL requires only 0.04~MB/round (10K-param MLP, 0.8~MB total). Imaging (ResNet-18, 11.2M params) requires 44.7~MB/round; Top-$k$ sparsification at 5\% achieves 95\% bandwidth savings albeit with accuracy cost on small models (Supplementary Material, Table~XXI). Chest X-ray validation~\cite{chestxray2018} with ResNet-18~\cite{he2016deep}, GroupNorm, and FedBN~\cite{li2021fedbn} achieves 87.3\% accuracy (FedAvg, 5 clients, Dirichlet $\alpha{=}0.5$). On Chest X-ray, \textbf{personalization is counterproductive}: Ditto achieves only 80.0\% ($-$7.3pp) and HPFL 69.1\% ($-$18.2pp), with per-client classifier collapse (Table~XX). However, on Skin Cancer HPFL \textit{outperforms} FedAvg (68.5\% vs.\ 62.8\%, +5.7pp) with better fairness (Jain 0.987 vs.\ 0.900), while on Brain Tumor both achieve $\sim$46\% (Supplementary Material, Table~XXIV). This reveals a \textbf{nuanced modality-dependent effect}: personalization helps on smaller imaging datasets but is counterproductive when the global model has sufficient data for robust shared representations. On Breast Cancer, confusion matrix analysis (10 seeds) confirms this asymmetry from the opposite direction: FedAvg/FedProx/Ditto exhibit single-class collapse on 8/10 data partitions (21.5\% aggregated Malignant recall, with 0\% on 7 individual seeds), while HPFL avoids collapse on all seeds (78.7\% aggregated Malignant recall; Supplementary Material, Table~XIX). EHDS data permits should condition algorithm selection on analytics modality.
