...
\textbf{Communication \& imaging}: Tabular FL requires only 0.04~MB/round (10K-param MLP, 0.8~MB total). Imaging (ResNet-18, 11.2M params) requires 44.7~MB/round; Top-$k$ sparsification at 5\% achieves 95\% bandwidth savings albeit with accuracy cost on small models (Supplementary Material, Table~XX). Chest X-ray validation~\cite{chestxray2018} with ResNet-18~\cite{he2016deep}, GroupNorm, and FedBN~\cite{li2021fedbn} achieves 87.3\% accuracy (FedAvg, 5 clients, Dirichlet $\alpha{=}0.5$). Crucially, \textbf{personalization is counterproductive on imaging}: Ditto achieves only 80.0\% ($-$7.3pp) and HPFL fails catastrophically at 69.1\% ($-$18.2pp), while FedLESAM matches FedAvg exactly (87.8\%). This reveals a \textbf{modality-dependent personalization effect}: lightweight tabular models benefit critically from personalization, but deep imaging models generalize better with global aggregation. On Breast Cancer, confusion matrix analysis confirms this asymmetry from the opposite direction: FedAvg/FedProx/Ditto predict \textit{only} the majority class (0\% Malignant recall), while HPFL achieves 91.8\% Malignant recall (Supplementary Material, Table~XIX). EHDS data permits should condition algorithm selection on analytics modality.
...
\textbf{Evaluation}: The tabular MLP ($\sim$10K parameters) produces a nearly convex loss landscape where server-side strategies (FedLESAM, FedExP, FedLC) converge to FedAvg-equivalent solutions. Confusion matrix analysis confirms this collapse is clinically severe: on Breast Cancer, FedAvg/FedProx/Ditto predict \textit{only} the majority class (0\% Malignant recall), while HPFL uniquely detects 91.8\% of Malignant samples (Supplementary Material, Table~XIX). A deeper model ($\sim$110K parameters, 4 hidden layers) does \textit{not} break this collapse (Table~XV), nor does increasing local epochs from $E$=1 to $E$=20 (Table~XVI). \textbf{Scalability} experiments extending to K=100 clients confirm that personalized algorithms degrade by only $-$0.8pp (vs.\ $-$4.7pp), with the personalization gap \textit{widening} from 11.7pp to 15.9pp (Tables~XVII--XVIII). DP at $\varepsilon{=}10$ imposes $<$0.4pp cost across all algorithms (Table~XXI); RDP composition provides 3.6$\times$ tighter privacy bounds than naive composition, enabling practical multi-round budgets (Figure~S16). On imaging, HPFL fails catastrophically (69.1\% vs.\ FedAvg 87.3\%) and FedLESAM offers no improvement over FedAvg, confirming a modality-dependent personalization effect. No direct comparison with Flower~\cite{beutel2023flower} or FLARE~\cite{nvflare2023} is provided: since no existing framework implements HDAB permit integration, Article~71 output filtering, or EHDS-compliant audit trails, such a comparison would reduce to pure FL algorithm performance---equivalent by design.
...
\textbf{Future work} should prioritize: (1)~empirical validation through HealthData@EU pilot integration with production EHR systems; (2)~citizen attitude studies examining FL acceptance and opt-out intentions across diverse European populations; (3)~larger-scale imaging evaluation across additional datasets and architectures, given the modality-dependent personalization effects identified in this work; (4)~economic sustainability modeling for HDAB operations and FL infrastructure.
