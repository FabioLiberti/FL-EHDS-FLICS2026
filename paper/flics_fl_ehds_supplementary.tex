...
The main paper's Table~I provides a feature-level comparison between FL-EHDS and existing FL frameworks (Flower~\cite{beutel2020flower}, FLARE~\cite{roth2023nvidia}, PySyft, FedML, TFF). A natural reviewer question is why we do not provide a direct \textit{experimental} comparison. This section explains the rationale.
...
The main paper argues that legal uncertainties---not technical barriers---constitute the critical blocker for FL adoption under the EHDS. This section provides concrete scenarios that illustrate how unresolved governance questions create implementation deadlocks that no engineering solution can circumvent.
...
(2)~\textit{DP as regularization on small data}: On Breast Cancer (569 samples), FedAvg with $\varepsilon{=}5$ achieves 78.7\%---\textit{higher} than the no-DP baseline of 52.3\% (+26.4pp). HPFL at $\varepsilon{=}1$ reaches 86.9\% vs.\ 74.1\% without DP (+12.8pp). The Gaussian noise acts as implicit regularization, preventing overfitting on this small dataset.
...
Table~\ref{tab:significance} reports Wilcoxon signed-rank test $p$-values comparing each algorithm against FedAvg, computed over 10 seeds (original seeds: 42, 123, 456, 789, 1024; additional seeds: 7, 31, 137, 577, 1337). With $n{=}10$ paired observations, the minimum achievable two-sided $p$-value is $2/2^{10} = 0.00195$.
...
\textbf{Key findings}: With 10 seeds, the statistical picture sharpens decisively. \textit{First}, \textbf{HPFL significantly outperforms FedAvg on all three datasets} ($p = 0.004$, $0.002$, $0.031$; all $< 0.05$), with improvements of +0.9pp on PTB-XL, +11.8pp on Cardiovascular, and +16.5pp on Breast Cancer---the only algorithm achieving significance across all datasets. \textit{Second}, \textbf{Ditto significantly outperforms FedAvg on Cardiovascular and Breast Cancer} ($p = 0.002$, $0.016$), with large effect sizes (+11.9pp, +19.0pp), though not on PTB-XL ($p = 0.49$, $\Delta = +0.2$pp). \textit{Third}, \textbf{five algorithms collapse to FedAvg-equivalent performance}: FedLESAM produces identical results on all datasets; FedExP and FedProx are identical on 2/3 datasets. This confirms that only methods maintaining separate local models (Ditto, HPFL) differentiate on the near-convex tabular MLP landscape. \textit{Fourth}, the \textbf{pooled analysis} across all 30 paired observations yields $p < 0.001$ for both Ditto and HPFL, providing strong evidence that personalized FL methods are superior for EHDS tabular analytics.
...
\bibitem{roth2023nvidia}
H. R. Roth \textit{et al.}, ``NVIDIA FLARE: Federated learning from simulation to real-world,'' \textit{arXiv preprint arXiv:2210.13291}, 2023.
