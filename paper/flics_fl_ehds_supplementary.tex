...
PX & FedAvg & 91.9\tiny{$\pm$0.5} & 52.3\tiny{$\pm$13.3} & 84.2\tiny{$\pm$4.3} & \textbf{92.4}\tiny{$\pm$0.3} & 92.4\tiny{$\pm$0.3} \\
 ...
 & HPFL & 92.6\tiny{$\pm$0.3} & 87.1\tiny{$\pm$2.8} & 90.6\tiny{$\pm$2.2} & 92.4\tiny{$\pm$0.4} & \textbf{92.7}\tiny{$\pm$0.2} \\
\midrule
...
\begin{table}[t]
\centering
\caption{Wilcoxon signed-rank $p$-values vs FedAvg baseline. $\dagger$: $p < 0.05$, *: $p < 0.10$. Computed over 5 seeds.}
...
\textbf{Key finding}: With 5 seeds, HPFL achieves marginal significance ($p = 0.062$, $< 0.10$) against FedAvg on \emph{all three datasets}---the only algorithm to do so consistently. Ditto reaches marginal significance on Cardiovascular and Breast Cancer ($p = 0.062$), where personalization yields large effect sizes (11.4pp and 26.8pp respectively). The $p = 0.062$ value represents the minimum achievable with the Wilcoxon test over 5 paired observations; the consistent directional advantage across all datasets and metrics provides strong practical significance.
...
Figures~\ref{fig:tab_convergence}--\ref{fig:tab_best_config} present visual analysis of the 1,410 tabular experiments. All plots are auto-generated by the benchmark analysis suite.
