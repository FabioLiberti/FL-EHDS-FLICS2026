...
\begin{enumerate}
    ...
    \item \textbf{Experimental Validation}: Evaluation on real clinical datasets demonstrating that algorithm selection produces 18.7pp accuracy differences and personalized FL narrows the centralized-federated gap to 6.6pp.
\end{enumerate}
...
\textbf{Datasets}: (1)~\textit{Heart Disease UCI}~(920 patients from 4 international hospitals: Cleveland, Hungarian, Swiss, VA Long Beach)---13 clinical features, binary cardiac disease diagnosis. The natural hospital partitioning creates authentic non-IID conditions. (2)~\textit{Diabetes 130-US}~(101,766 encounters from 130 US hospitals)---22 clinical features; binary 30-day readmission prediction with severe class imbalance ($\sim$11\% positive rate), partitioned via Dirichlet $\alpha$=0.5. \textbf{Model}: HealthcareMLP (2-layer, 64/32 hidden, ReLU, dropout 0.3, $\sim$10K parameters). \textbf{Configuration}: 20 rounds, 3 local epochs, batch size 32, Adam optimizer (lr=0.01). All results are mean $\pm$ std over 3 seeds.
