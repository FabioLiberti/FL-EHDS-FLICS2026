...
\begin{enumerate}
    ...
    \item \textbf{Experimental Validation}: 1,740 experiments on 5 tabular clinical datasets (including the European-origin PTB-XL ECG) with differential privacy ablation across $\varepsilon \in \{1, 5, 10, 50\}$, demonstrating that algorithm selection produces 18.7pp accuracy differences and personalized FL narrows the centralized-federated gap to 6.6pp.
\end{enumerate}
...
Table~\ref{tab:main_results} presents FL algorithm comparison on Heart Disease and Diabetes. Table~\ref{tab:tabular_main} extends evaluation to PTB-XL, Cardiovascular, and Breast Cancer with 7 algorithms including FedLESAM and HPFL---1,515 total experiments across heterogeneity sweeps, client scaling, learning rate sensitivity, and differential privacy ablation (see Supplementary Material, ``Extended Tabular Experiment Results'').
...
\textbf{Privacy-utility tradeoff}: A comprehensive ablation across $\varepsilon \in \{1, 5, 10, 50\}$ (180 experiments; Gaussian mechanism, $C{=}1.0$, $\delta{=}10^{-5}$) reveals that personalized methods are remarkably DP-robust: at $\varepsilon{=}10$, Ditto and HPFL lose $<$1pp on PTB-XL, while FedAvg collapses ($-$39.6pp at $\varepsilon{=}1$). Privacy is essentially free at $\varepsilon{=}10$ ($<$2pp cost), satisfying EHDS Article~50 SPE requirements. \textbf{Article~71 opt-out}: simulating citizen opt-out at 5--30\% rates (225 experiments) confirms $<$1pp accuracy loss on adequately-sized datasets even at 30\% opt-out---citizen privacy rights are compatible with FL quality. Full privacy results in Supplementary Material.
...
\textbf{Clinical imaging}: The framework extends to medical imaging using ResNet-18~\cite{he2016deep} with GroupNorm and FedBN~\cite{li2021fedbn} on Chest X-ray~\cite{chestxray2018}, Brain Tumor MRI, and Skin Cancer datasets. Dataset configurations and imaging pipeline details are provided in the supplementary material. Preliminary imaging experiments confirm framework functionality; comprehensive imaging evaluation across all 7 algorithms is reserved for future work.
...
\textbf{EU Policymakers}: The March 2027 delegated acts should address FL-specific scenarios including gradient privacy, multi-party controller allocation, and model anonymity thresholds. \textbf{National Authorities}: Early HDAB capacity investment is essential; the 2--3 year Nordic advantage~\cite{tehdas2024ready} demonstrates that governance capacity may prove more constraining than technical infrastructure. \textbf{Healthcare Organizations}: Preparation cannot wait for 2029---accelerating FHIR compliance beyond the 34\% baseline~\cite{hussein2025interop} and assessing FL infrastructure readiness are immediate priorities. Deployment should follow phased milestones: foundation and pilots (2025--26), delegated acts clarification (2027), production scaling (2028--29), and full cross-border operation (2029--31). FL-EHDS's modular governance layer enables incremental binding to HDAB services as they become available.

\subsection{Limitations}

Our evaluation uses retrospective public datasets; real-world integration with production EHR systems remains essential future work. The tabular MLP ($\sim$10K parameters) produces a nearly convex loss landscape where server-side strategies (FedLESAM, FedExP, FedLC) converge to FedAvg-equivalent solutions; deeper models are expected to differentiate these algorithms. While PTB-XL provides authentic European multi-site heterogeneity (52 sites), true cross-border validation requires datasets spanning multiple Member States with distinct national healthcare systems---a resource that does not yet exist publicly and that the EHDS itself aims to enable. The governance layer operates as a simulation backend; binding to actual HDAB REST/gRPC endpoints requires only configuration changes (endpoint URLs, mTLS certificates), not architectural modifications. No direct experimental comparison with Flower~\cite{beutel2020flower} or FLARE~\cite{roth2023nvidia} is provided: since no existing framework implements HDAB permit integration, Article~71 output filtering, or EHDS-compliant audit trails, such a comparison would reduce to pure FL algorithm performance---equivalent by design across frameworks implementing the same canonical algorithms. FL-EHDS's contribution lies in the governance-technical integration layer, not in algorithmic novelty (detailed analysis in Supplementary Material).
