...
Chest X-ray experiments use 4 algorithms (FedAvg, FedLESAM, Ditto, HPFL) $\times$ 3 seeds = 12 experiments. Brain Tumor and Skin Cancer use 2 algorithms (FedAvg, HPFL) $\times$ 1 seed as qualitative validation of the modality-dependent personalization effect; statistical significance is established on the tabular benchmarks (10 seeds, $p < 0.001$).
...
\begin{table}[htbp]
\centering
\caption{Brain Tumor MRI (4-class) and Skin Cancer (binary): FedAvg vs HPFL with ResNet-18. Early stopping. Brain Tumor: single seed (42), 10 rounds; Skin Cancer: 3 seeds (42, 123, 456), 20 rounds. Best accuracy reported.}
...
\midrule
\multirow{2}{*}{Skin Cancer} & FedAvg & 63.0$\pm$9.2 & 0.836 & 14.0 \\
 & HPFL & 62.9$\pm$7.5 & \textbf{0.971} & 10.0 \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize{Brain Tumor: single seed (42), 10 rounds, lr=0.0005. Skin Cancer: mean$\pm$std over 3 seeds (42, 123, 456), 20 rounds, lr=0.001. FedAvg and HPFL are statistically indistinguishable on Skin Cancer ($-$0.1pp), though HPFL maintains superior fairness (Jain 0.971 vs.\ 0.836).}
\end{table}

\textit{Implications}: The three-dataset imaging evaluation reveals that \textbf{personalization offers no advantage on imaging}. On Chest X-ray (5,856 samples), FedAvg dominates (+18.2pp over HPFL). On Skin Cancer (3,297 samples, 3 seeds, 20 rounds), FedAvg and HPFL are statistically indistinguishable (63.0$\pm$9.2\% vs.\ 62.9$\pm$7.5\%, $-$0.1pp), though HPFL maintains superior fairness (Jain 0.971 vs.\ 0.836). On Brain Tumor (7,023 samples, 4-class), both achieve $\sim$46\%. The high variance across seeds on Skin Cancer (FedAvg: 53.8--72.1\%, HPFL: 55.4--70.4\%) underscores the sensitivity of imaging FL to data partitioning---a practical consideration for EHDS deployment. HPFL's personalized classifier heads provide no benefit on imaging tasks, where CNN features require global aggregation for robust shared representations.
